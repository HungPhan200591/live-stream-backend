...
| [`commit-livestream-change`](../../.agents/skills/commit-livestream-change/SKILL.md) | Một `gpt-5.6-terra` subagent `low` chạy fast path tối thiểu để sinh message và tạo local commit; direct call commit tất cả, không push. | Người dùng gọi trực tiếp skill hoặc yêu cầu local commit nhanh. | Chỉ cần sinh commit message hoặc review read-only. |
