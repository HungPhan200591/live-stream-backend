...
| **Tổng cài đặt** |  | **19** | 16 skill được expose trong session hiện tại; skill mới cần session discovery để xuất hiện; `review-agent` chỉ được cài trên filesystem |
...
| [`commit-livestream-change`](../../.agents/skills/commit-livestream-change/SKILL.md) | Fast path local duy nhất: script chuẩn bị, tối đa 40 dòng diff để sinh message rồi tạo local commit; không subagent, không test, không push. | Người dùng gọi trực tiếp skill hoặc yêu cầu local commit nhanh. | Chỉ cần sinh commit message hoặc review read-only. |
