...
| [`commit-livestream-change`](../../.agents/skills/commit-livestream-change/SKILL.md) | Kiểm tra staged diff, secret và validation evidence rồi tạo Git commit cục bộ; gọi skill đứng một mình mặc định commit toàn bộ thay đổi hiện tại thành một commit và không push. | Người dùng gọi trực tiếp skill, yêu cầu commit, local commit hoặc commit các file/change cụ thể. | Chỉ cần sinh commit message hoặc review read-only. |
