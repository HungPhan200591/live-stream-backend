...
| [`commit-livestream-change`](../../.agents/skills/commit-livestream-change/SKILL.md) | Dùng `gpt-5.6-terra` reasoning `low` và bundled fast path để kiểm tra/stage, sinh message rồi tạo một commit cục bộ; gọi skill đứng một mình mặc định commit toàn bộ và không push. | Người dùng gọi trực tiếp skill, yêu cầu commit, local commit hoặc commit scope cụ thể. | Chỉ cần sinh commit message hoặc review read-only. |
