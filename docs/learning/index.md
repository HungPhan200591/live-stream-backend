...
| Latest evidence | Toàn bộ `52/52` theory/deep-dive hiện có là `TEACHABLE_DRAFT` (`18` Java, `10` Spring, `4` HTTP/API, `4` Resilience, `2` Testing, `10` PostgreSQL/JPA/SQL, `2` Redis, `2` Security), với `62` Mermaid diagrams. Security foundation đã nối `2/7` question-bank slices (`23/23` câu của token/session và authorization); `5` slices còn thiếu vẫn được ghi rõ, không coi coverage hoàn tất. Mọi batch chỉ là tài liệu chuẩn bị, không active case. Learner self-check, reproducer, test và runtime evidence vẫn `NOT RUN`, nên cursor/checkpoint/depth không đổi. Livestream use-case catalog vẫn có `41` scenario và owner mapping `43/43` |
...
| Security, realtime và abuse resistance | Stage 0, 7 | `2/7` core theory là `TEACHABLE_DRAFT`: token/session (`SEC-01`) và request/method authorization (`SEC-06`), đã nối `23/23` câu tương ứng; còn thiếu session revocation, secret exposure, webhook HMAC, OAuth2/OIDC và abuse/supply-chain. SEC-01 vẫn `PAUSED`, evidence `NOT RUN` |
