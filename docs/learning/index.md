...
| Latest evidence | Livestream use-case catalog đã chuẩn hóa `41` scenario (`9` capstone + `32` supporting) từ Foundation tới Expert, có priority/difficulty và owner mapping cho `43/43` learning item; chưa case nào được kích hoạt từ catalog. Question-bank preview corpus đã phủ roadmap và capability rubric `3.1-3.24`; tất cả vẫn `DRAFT`/`UNANSWERED`/`NOT RUN` và không được tính là learning evidence. JDK-01 learner review cùng mọi Java 21 compatibility/build/JFR/experiment evidence vẫn `NOT RUN` |
