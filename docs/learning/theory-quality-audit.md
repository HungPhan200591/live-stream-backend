...
| Security | 5 | 0 | Stage 0 trust-boundary theory nối 5/7 security slices; OAuth2/OIDC và abuse/supply-chain còn là quality backlog |
| **Tổng** | **55** | **0** | Không file nào được coi là learner/evidence completed |
...
- Mermaid chỉ dùng khi quan hệ khó hiểu bằng prose; toàn corpus hiện có `65` diagrams;
...
Sau migration, cặp theory `TEST-01`, batch `10` PostgreSQL/JPA/SQL, `2` Redis và `5/7` Security theory được tạo theo yêu cầu chuẩn bị trước, đạt `TEACHABLE_DRAFT` ngay từ đầu và nối question bank tương ứng. Security coverage chưa hoàn tất cho tới khi OAuth2/OIDC và abuse/supply-chain được dạy đầy đủ. Việc chuẩn bị trước không thay execution order: learning cursor vẫn ở `JDK-01/THEORY_CORE` và không yêu cầu learner đọc cả corpus.
