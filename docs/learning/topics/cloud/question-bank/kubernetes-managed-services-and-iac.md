...
> Related theory: [Core](../theory/core/kubernetes-managed-services-and-iac.md) · [Deep-dive](../theory/deep-dives/autoscaling-retry-collapse-iac-drift-and-regional-recovery.md)<br>
> Updated: `2026-07-26`
...
**Answer outline:** Pod chứa container; Deployment quản lý replica và rollout; Service tạo discovery/load balancing ổn định tới pod; Ingress/Gateway route HTTP/TLS bên ngoài qua controller. Các object này không tự bảo đảm correctness của ứng dụng.<br>
...
**Evidence:** Theory [core](../theory/core/kubernetes-managed-services-and-iac.md) + [deep-dive](../theory/deep-dives/autoscaling-retry-collapse-iac-drift-and-regional-recovery.md); case `CLOUD-01 NOT CREATED`; tests `NOT RUN`; note `NOT CREATED`.<br>
...
**Answer outline:** ConfigMap chứa config không nhạy cảm; Secret API phân phối dữ liệu nhạy cảm nhưng vẫn cần encryption, RBAC và rotation; workload identity cấp credential cloud/service ngắn hạn mà không giữ static key.<br>
...
**Answer outline:** HPA đổi số pod theo metric; VPA đề xuất/đặt resource request và thường cần restart; cluster autoscaler thêm/bớt node theo capacity pending. App, bottleneck và state phải hỗ trợ scale.<br>
**Required trade-offs:** Autoscaling tiết kiệm cost nhưng phản ứng trễ với burst và có thể khuếch đại tải xuống dependency.<br>
...
**Answer outline:** IaC code/provider tính plan từ state và resource thật; drift là khác biệt do thay đổi thủ công/bên ngoài. State remote phải lock/encrypt, plan được review và import có kiểm soát.<br>
...
**Answer outline:** So HA, backup, patching, scaling, support; feature/limit hiệu năng, data residency, lock-in/egress, staffing và incident ownership. Benchmark semantics quan trọng.<br>
...
**Answer outline:** Đo startup/resource/load, đặt request/headroom/limit thực tế, tách probe, graceful drain, `maxUnavailable`/surge và PDB theo capacity node/zone; test throttling và eviction.<br>
...
**Answer outline:** Dùng CI identity ngắn hạn, state được bảo vệ, provider/module đã pin, lint/security/policy check, human approval cho production/destructive change, môi trường theo stage và kế hoạch backup/rollback/restore; redact secret trong plan.<br>
...
**Answer outline:** Lập bản đồ service theo zone/region, replication và RPO, DNS/GSLB, quota/egress, identity/control-plane; chèn lỗi mất zone và diễn tập region recovery thay vì chỉ nhân các con số SLA.<br>
...
**Answer outline:** Dùng PaaS/managed container khi chỉ có ít service tiêu chuẩn; chọn Kubernetes khi nhu cầu scheduling, extensibility hoặc platform nhiều team biện minh cho đội chuyên trách. Tính security, upgrade, on-call, TCO và exit path.<br>
...
**Answer outline:** Đối chiếu HPA, thời điểm pod start/readiness, retry QPS, pool connection và database saturation; giới hạn concurrency theo pod/toàn hệ thống, retry budget, tốc độ scale, warmup và DB admission; shed traffic rồi phục hồi dần.<br>
